...
"""
把 Hugging Face 的 cnn_dailymail 数据集导出为 CSV 文件。

Hugging Face 的 cnn_dailymail 是一个著名的 新闻摘要数据集，包含：
article：新闻正文；
highlights：人工撰写的新闻摘要；
id：样本编号；
分为 train、validation、test 三个子集。
"""

# ======== 配置（按需修改）========
SAVE_PATH = Path("/Users/yuanjie05/Downloads/cnn_dailymail_full.csv")  # 也可改成 .csv.gz
