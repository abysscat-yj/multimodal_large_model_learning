...
class Config:
    # ---------- 模型结构 ----------
    ACTIVATION: str = "relu"  # 激活函数: relu | gelu | tanh | sigmoid
    NORM: str = "batch"  # 归一化层: batch | layer | group | rms | none
    GROUPS: int = 8  # GroupNorm分组数（通道须能被整除）
    DROPOUT_P: float = 0.1  # Dropout 概率

    # ---------- 优化配置 ----------
    OPTIMIZER: str = "adamw"  # 优化器类型: sgd | momentum | rmsprop | adam | adamw
    LR: float = 3e-4  # 学习率
    WEIGHT_DECAY: float = 0.01  # 权重衰减项（L2正则）

    # ---------- 学习率调度 ----------
    SCHEDULER: str = "cosine_warmup"  # 调度器策略: none | step | cosine | cosine_warmup | reduce_on_plateau
    STEP_DECAY_GAMMA: float = 0.5
    STEP_DECAY_EVERY: int = 5
    COSINE_WARMUP_STEPS: int = 200  # warmup步数（以step计）
    ...
    # ---------- 损失函数 ----------
    LOSS: str = "cross_entropy"  # 损失函数: cross_entropy | mse (分类建议cross_entropy)

    # ---------- 数据配置 ----------
    NUM_CLASSES: int = 10
    IMG_SIZE: int = 32
    TRAIN_SAMPLES: int = 20000  # 训练样本总数
...
# =========================
# 数据：FakeData
# 随机生成每张图片（像素值是随机数）；随机生成一个标签（0～NUM_CLASSES-1）；每次取的数据之间没有任何规律。
# 随机生成图片和标签。每个样本都是噪声，没有规律。仅用于测试训练流程，不可能收敛。
# =========================
mean = (0.5, 0.5, 0.5)
std = (0.5, 0.5, 0.5)
...
# ===============================================================
# 小型CNN结构定义，卷积层 → 激活 → 池化 → 卷积层 → 激活 → 池化 → 全连接 → 输出
# ===============================================================
...
# ===============================================================
# 损失函数 / 优化器 / 学习率调度器 构造函数
# ===============================================================
def make_criterion(name: str):
    """损失函数衡量模型输出与真实标签的差距"""
    name = name.lower()
    # CrossEntropyLoss适合分类任务，模型输出 logits，自动做 softmax
    if name == "cross_entropy":
        return nn.CrossEntropyLoss()
    # MSELoss适合回归任务，直接计算均方误差
    if name == "mse":
        return nn.MSELoss()
...
def main():
    ...
    for epoch in range(1, CFG.EPOCHS + 1):
        t0 = time.time()
        train_loss, train_acc, global_step = train_one_epoch(model, train_loader, optimizer, criterion, scheduler,
                                                             global_step)
