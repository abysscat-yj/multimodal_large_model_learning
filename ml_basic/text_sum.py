...
# ROUGE：评价“机器摘要”和“人工摘要”相似程度的指标，ROUGE-L：基于“最长公共子序列”的一个版本，越高表示越像。
...
# ======== 配置 ========
DATA_CSV = "/Users/yuanjie05/Downloads/cnn_dailymail_full.csv"  # CNN/DailyMail数据集路径
...
def topk_diverse(indices_scores: List[Tuple[int, float]], sent_vecs, k=3, sim_th=0.6):
    """摘要句子去重，在多个候选句子中，挑 k 个相似度低的句子，让摘要更加多样化。"""
    picked = []
...
def summarize(text: str, artifacts, topk=TOPK_SENT) -> str:
    ...
    if not sents:
        return ""
    tfidf_sent = artifacts['tfidf_sent'];
    tfidf_doc = artifacts['tfidf_doc'];
    clf = artifacts['clf']
