...
"""
目标：训练一辆“车”学会在一条有红绿灯的直路上前进，红灯不要闯、绿灯再走，尽量在给定步数内到达终点并获得奖励。

方法：用Q-learning（无模型、表格型）学习状态→动作的价值表 Q[s,a]；训练好后用 Matplotlib 做一个可交互动画展示策略。

奖励设计与策略直觉：
到达终点 +5：驱动目标导向。
红灯前闯灯 -10：强烈阻止危险动作（在 pos>=light_pos-1 的时候闯红灯会被重罚）。
等待 -0.1：避免一直停着不动；但如果红灯在前、前进一步会吃 -10，那么等一会儿的 -0.1 反而更好。
折扣 γ=0.95：鼓励尽早到达，但仍考虑未来回报。
结果：在红灯前等待，绿灯亮了再走；一路绿灯则尽快前进。
"""


# 播放过程中：g键，在“学到的策略(LEARNED)/随机策略(RANDOM)”之间切换，RANDOM模式下会有可能会闯灯；
# r键，重置；q键，退出。“空格”键：内部动作为STOP, 此时按n前进一帧；


# ---- 环境定义 ----
@dataclass
class TLConfig:
    road_len: int = 8  # 路的长度（格子数，位置从 0 到 road_len；到达或越过即成功）
    light_pos: int = 5  # 红绿灯位置
    cycle_red: int = 20  # 红灯时长（tick）
    cycle_green: int = 20  # 绿灯时长（tick）
    max_steps: int = 60  # 每局最多步数
    gamma: float = 0.95  # 折扣因子，0~1 之间的小数，γ 越大：越重视长期回报；γ 越小：越倾向于短期收益，第一个奖励不打折；后面的奖励每往后一步，都要乘一个 γ（折扣）
...
# Q-learning快速训练
# Q-learning 是一种强化学习算法，用来让智能体（agent，比如小车）通过「试错」学会如何在环境中行动以获得最大奖励。
# 通过一个Q 表 (Q-table) 来记录「在每个状态下执行某个动作的长期价值」
...
class TLRenderer:
    ...
    def _setup_canvas(self):
        ...
        self.title = self.ax.set_title(
            "Training loaded. Playing learned policy… (space: pause / n: step / g: toggle / r: reset / q: quit)")
